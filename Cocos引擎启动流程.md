...
```   
##### 5.调用当前场景的渲染方法   renderScene(_runningScene, _renderer);   
5.1 加载当前Camera     
...
5.3 调用visit函数，该函数主要是递归调用当前节点下子节点的visit(),然后调用自身的draw函数将相应的Command命令将渲染请求加入当前的renderer的渲染队列中。 
   
```     
//visit the scene   
visit(renderer, transform, 0);
```   
5.3.1 对当前子节点排序  sortAllChildren();
5.3.2 递归调用当前localZOrder小于当前节点的子节点的visit().    
...
```   
5.3.3 调用自身的draw()把指定的渲染命令添加到当前的渲染队列   
...
```   
5.4 调用渲染引擎进行渲染 renderer->render();   
5.5 处理visit完毕事件，通知visit完毕。_eventDispatcher->dispatchEvent(_eventAfterVisit);      
5.6 调用渲染引擎进行渲染  _renderer->render();  绘制内容   
5.7  _eventDispatcher->dispatchEvent(_eventAfterDraw); 通知绘制完成事件
5.8 调用OpenGLView平台提供的屏幕显示方法    
...
所有操作做完了，底层OpenGL调用不同平台对OpenGLES的支持函数，把在OpenGL里的缓存帧内容显示在屏幕上。
